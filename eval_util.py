...
def failure_cases(predictions, labels, k=20):
  """Get the top_k for a 1-d numpy array. Returns a sparse list of tuples in
  (prediction, class) format"""
  ...
  for pred_batch, label_batch in zip(predictions, labels):
    indices = numpy.argpartition(label_batch, -k)[-k:]
    # print indices.shape, indices
    for idx in indices:
      if pred_batch[idx]!=label_batch[idx]:
        results[idx] +=1
  return results
...
class EvaluationMetrics(object):
  ...
  def __init__(self, num_class, top_k):
    ...
    self.top_k = top_k
    self.num_examples = 0
    self.results = [0 for i in range(num_class)]
  ...
  def accumulate(self, predictions, labels, loss):
    ...
    temp = failure_cases(predictions, labels, k=20)
    for idx, item in enumerate(self.results):
      self.results[idx] += temp[idx]
  ...
  def get(self):
    ...
    return {"avg_hit_at_one": avg_hit_at_one, "avg_perr": avg_perr,
            "avg_loss": avg_loss, "aps": aps, "gap": gap, "results" : self.results}
